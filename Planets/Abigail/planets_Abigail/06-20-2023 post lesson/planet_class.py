...
class Planet():
    ...
    def display_distance_to_sun(self, screen, x, y):
        pygame.draw.circle(screen, self.color, (10 + 50, 88 + y), self.radius)

        match self.name:
            case "Mercury":
                distance_text = Planet.FONT_LST_16.render(f"{self.name}  {math.floor(self.distance_to_sun/1000)} km",
                                                          True, Planet.BLACK)
            case "Venus":
                distance_text = Planet.FONT_LST_16.render(f"{self.name}   {math.floor(self.distance_to_sun/1000)} km",
                                                          True, Planet.BLACK)
        ...
        screen.blit(distance_text, (x + 100, 78 + y))
