...
class Planet:
	...
	def draw(self, win):
		...
		if not self.sun:
			distance_text = FONT.render(f"{math.floor(self.distance_to_sun/1000)}km", 1, WHITE)

			win.blit(distance_text, (x - distance_text.get_width()/2, y - distance_text.get_height()/2 - 2*self.radius -5))

			planet_text = FONT.render(self.name, 1, WHITE)
			win.blit(planet_text, (x - planet_text.get_width()/2, y - planet_text.get_height()/2 + 2*self.radius))
...
def main():
	...
	mercury = Planet("mercury", 0.387 , 0, 8, DARK_GREY, 3.30 * 10**23, -47.4 * 1000, 1)
	# mercury.y_vel = -47.4 * 1000

	venus = Planet("venus",0.723 , 0, 14, WHITE, 4.8685 * 10**24, -35.02 * 1000, 1)
